...
![Issues tab](img/githubIssues.png)

Appropriate tagging will aid in error tracing.  
* Bugs: When resolved, staff should closed issue with sufficiently detailed comment for potential future reference, and include the git commit hash of the version pre/post-fix if appropriate.     
* Features request: Needs and suggestions for improvement features can be logged under issues as well.  As resources allows, the feature work can be initiated as a new feature branch.  Again, upon completion, the git commit hash should be logged in the closure comment.  
